...
type ProtocolEncoder interface {
	Encode(s *Session, packetid int, packet interface{}, w io.Writer) (data []byte, err error)
...
func (dec *DefaultProtocolEncoder) Encode(s *Session, packetid int, packet interface{}, w io.Writer) (data []byte, err error) {
...
func (dec *DefaultProtocolEncoder) CutAndSendPacket(s *Session, data []byte, w io.Writer) (err error) {
	if dec.PacketCutor != nil {
		...
		for i := 0; i < len(slices); i++ {
			_, err = dec.Encode(s, packid, slices[i], w)
			if err != nil {
				return
			}
			if s.scpl != nil {
